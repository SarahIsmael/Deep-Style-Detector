...
@author: sarah Ismael
In this program we load the e-codices dataset 8450 images, Implementing some Image preprocessing 
...
def parse_commandline_args():
    """To Run: python DeepStyleDetector.py --nrows 224 --ncols 224 --epoches 10 
    --batch_size 32 --images_dir 
    train_images_1Apr/ --output_dir results224nc224 --output_label test1 
    --nkernals 32 --dropout 0.25 --num_patches 100"""
    
    parser = argparse.ArgumentParser(description='Deep Learning Model')
    #image processing parameters
    parser.add_argument('--nrows', type=int, default=224, help='number of rows')
    parser.add_argument('--ncols', type=int, default=224, help='number of column')
    ...
    parser.add_argument('--num_patches', type=int, default=100, help='num_patches from each image')
    ...
    parser.add_argument('--dense_unit', type=int, default=1028, help='unit size for dense layer')
    
    #data reading params
    parser.add_argument('--skip_read_existing_arrays', action='store_const', const=True, help='')
    parser.add_argument('--images_dir', default = 'train_images_1Apr', help='input images')
    parser.add_argument('--output_dir', default = str(datetime.datetime.now()).replace(" ", "").replace(":", ""), help='output directory path')
    parser.add_argument('--output_label', default = "test1", help='output label')
...
def get_x_y_from_dir(dir, labels):
    ...
    for label in labels:
        ...
        for image_file in images:
            img_arr = cv2.imread(dir+'/'+label+'/'+image_file, cv2.IMREAD_GRAYSCALE)
...
def preprocess_and_seperate(labels):
    ''' In this function we seperate the dataset to three sets:(training set,validation set and test set)
    each set sends to preprocess function and store it in its folder
    '''
    ...
    for label in labels:
        try:
            ...
            pass
    # Here we paralelling the Image pre-processing by using thread
    ...
    train_images = image_names[0:int(len(image_names)*0.7)]
    validation_images = image_names[int(len(image_names)*0.7):int(len(image_names)*0.8)]
...
def load_testdata(test_dir):
    """load data into keras Generator"""
...
if __name__ =='__main__':
    ...
    labels = ["cursiva", "carol", "bastarda", "gotica", 
              "textualis", "spaetcar", "textura"]
    ...
    #sns.heatmap(df_cm, annot=True, vmin=0, vmax=600, fmt="d")
